...
class MediaEditor(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # Label
        self.label = Label(text="Drag and Drop Media Files Here or Select from File Chooser", size_hint=(1, 0.1))
        ...
        # Bind the window to accept file drops using the updated event
    ...
    def on_file_drop(self, window, file_path, x, y):
        ...
        print(f"File dropped: {file_path} at position ({x}, {y})")

        # Update the selected file path
        self.selected_file = file_path
        self.label.text = f"Selected file: {os.path.basename(file_path)}"

    def edit_media(self, action):
        """Process the selected file for the given action."""
        if not self.selected_file and not self.file_chooser.selection:
            self.show_popup("Error", "No file selected")
            return
        
        # If file is selected via drag-and-drop, use it
        file_path = self.selected_file if self.selected_file else self.file_chooser.selection[0]
